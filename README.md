...
このツールは**毎週の収録〜公開を最小の手数で回す**ことに特化しています。多機能なPodcast編集ソフトの置き換えは目指しておらず、以下は**あえて**搭載していません。

- 途中のカット・並べ替え編集（対応するのは前後トリミングのみ）
- EQ・コンプレッサー等の手動エフェクト（音量調整とクリップ防止は自動）
- エピソードの保存・管理機能（1収録＝1セッション、書き出したら完了）

細かく編集したい回だけ、書き出したMP3を Audacity 等で仕上げる使い分けを想定しています。
...
- **収録中はOSの通知をオフに**（おやすみモード/集中モード推奨）。システム音声トラックには「PCで鳴った音すべて」が入るため、SlackやメールなどのOS通知音も録音されてしまいます。Discordのサウンド付き画面共有で流した動画等の音も同様に入ります（意図的に番組内で使う場合を除き、収録中は音を出さない）
- **（Windows）Discordの音声出力は既定の出力デバイスのままに。** システム音声の取得は既定デバイスのループバックを拾うため、Discordだけ別のデバイスに出していると相手の声が録音されません（macOSはシステム全体の音を拾うため影響なし）
- Discord上で自分が画面共有しながらの収録もOK。Discordの画面共有とこのツールの画面共有（音声取得）は独立していて干渉しません
...
| 設定・APIキー・テンプレート | localStorage | 設定画面の「削除」ボタン、または手動で消すまで保持 |
| 書き出したMP3/WAV | ダウンロードフォルダ | ― |

## APIキーの取り扱い

- キーは**このブラウザの localStorage にのみ**保存されます。開発者のサーバー等へ送信されることはありません（通信先は Gemini / OpenAI のAPIのみ）
- **削除**: 設定画面のキー欄横の「削除」ボタン → 保存。または DevTools → Application → Local Storage → `podcast-studio-settings` を削除
- **保存したくない場合**: 設定の「キーをこのブラウザに保存しない」をON。キーはメモリ上のみに保持され、タブを閉じる/再読み込みで消えます（共有PC向け）
- **注意（GitHub Pages利用時）**: localStorage はオリジン単位のため、`username.github.io` 配下の**同アカウントの他のPagesページとストレージが共有**されます。信頼できないコードを同アカウントで公開しない、またはカスタムドメインで分離してください
- **漏洩に備えたキー側の防御（推奨）**:
  - Gemini: Google Cloud コンソールでキーに「HTTPリファラー制限」（配信URLのみ許可）と「API制限」（Generative Language API のみ）を設定。無料枠キーなら課金被害はそもそも発生しません
  - OpenAI: プロジェクトスコープのキーを使い、月額上限（Budget）を設定
